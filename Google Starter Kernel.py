# Google Starter Kernel.py
# Google Starter Kernel from NCAA ML Competition 2019
...
# Import Data from subprocess import check_output
# print(check_output(["ls", "../input"]).decode("utf8"))
data_dir = '../input/'
df_seeds = pd.read_csv('/Users/Koby/PycharmProjects/NCAACompetition/Input/DataFiles/NCAATourneySeeds.csv')
...
df_sample_sub.to_csv('Starter-Kernel-Predictions.csv', index=False)

# Makes guesses on low confidence predictions, writes to other csv files
# Predicts the lower teamID will win with 61% confidence
df_sample_sub['Pred'].mask(df_sample_sub['Pred'].between(0.23, 0.77), other=0.64, inplace=True)
df_sample_sub.to_csv('Starter-Kernel-Predictions-lowGuess.csv', index=False)

# Predicts the higher teamID will win with 61% confidence
df_sample_sub['Pred'].mask(df_sample_sub['Pred'].between(0.64, 0.64), other=0.36, inplace=True)
df_sample_sub.to_csv('Starter-Kernel-Predictions-highGuess.csv', index=False)